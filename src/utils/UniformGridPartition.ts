...
export class UniormGridPartition {
  ...
  rebuild(s: SimState) {
    ...
    for (let i = 0; i < s.numParticles; i++) {
      const gx = Math.min(
        Math.floor(s.positionsX[i] * this.numDivisions),
        this.numDivisions - 1,
      );
      const gy = Math.min(
        Math.floor(s.positionsY[i] * this.numDivisions),
        this.numDivisions - 1,
      );
      const gz = Math.min(
        Math.floor(s.positionsZ[i] * this.numDivisions),
        this.numDivisions - 1,
      );

      const cellIdx =
        gx +
        gy * this.numDivisions +
        gz * this.numDivisions * this.numDivisions;
